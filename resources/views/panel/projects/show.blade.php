                                ...
                                <div class="modal fade" id="add_new_folder" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{route('panel_project_add_folder',['project'=>$project->id])}}" method="post">
                                                @csrf
                                ...
                                <div class="modal fade" id="add_new_url" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            ...
                                            <form action="{{route('panel_project_add_url',['project'=>$project->id])}}" method="post">
                                                @csrf
                            ...
                            <div class="mt-7 scroll scroll-pull">
                                        ...
                                        <div class="card card-body p-4 mb-3">
                                            @foreach($folder->urls as $url)
                                               <div class="@if(request('req') == $url->id) bg-light-primary p-1 @endif">
                                                   ...
                                                   <a class="ml-2 text-dark " href="{{route('panel_project_show',['project'=>$project->id])}}?folder={{$folder->id}}&req={{$url->id}}">{{$url->title}}</a>
                                               </div>
                                                @if(!$loop->last)
                                                    <hr>
                                                @endif
                                ...
                                @if($project->urls()->whereNull('folder_id')->exists())
                                        @foreach($project->urls()->whereNull('folder_id')->get() as $url)
                                            <div class="@if(request('req') == $url->id) bg-light-primary p-1 @endif">
                                                {!! url_method_badge($url->method) !!}
                                                <a class="ml-2 text-dark" href="{{route('panel_project_show',['project'=>$project->id])}}?req={{$url->id}}">{{$url->title}}</a>
                                            </div>
                                            @if(!$loop->last)
                                                <hr>
                                            @endif
                                    ...
                                    <div class="modal fade" id="req_edit_{{$req->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-lg" role="document">
                                            <div class="modal-content">
                                                ...
                                                <form action="{{route('panel_project_update_url',['url'=>$req->id])}}" method="post">
                                                    @csrf
                                                    <div class="modal-body form-row">
                                                        <div class="form-group col-md-6">
                                                            <label>انتخاب پوشه</label>
    ...
    <script>
        ...
        function delreq(id) {
            Swal.fire({
                ...
                if (result.value) {
                    window.open('/panel/projects/delete/url/'+id,'_self');
                }
            })
        ...
        function delfolder(id) {
            Swal.fire({
                ...
                if (result.value) {
                    window.open('/panel/projects/delete/folder/'+id,'_self');
                }
            })
